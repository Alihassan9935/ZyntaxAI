...
pub enum HotkeyError {
    ...
    #[error(
        "a global hotkey needs at least one modifier (Ctrl, Alt, Shift or Super), otherwise it \
         would fire while you are typing"
    )]
    NoModifier,
    #[error("a global hotkey needs a key as well as modifiers")]
    NoKey,
    #[error("'{0}' is not a key ZyntaxAI recognises")]
    UnknownKey(String),
...
mod tests {
    ...
    fn parses_the_default_hotkey() {
        let hotkey = Hotkey::parse("Ctrl+Alt+T").expect("parses");
    ...
    fn normalises_case_and_spacing() {
        let a = Hotkey::parse("ctrl + alt + t").expect("parses");
        let b = Hotkey::parse("CTRL+ALT+T").expect("parses");
        assert_eq!(a, b);
    }

    #[test]
    fn modifier_order_does_not_matter() {
        let a = Hotkey::parse("Alt+Ctrl+T").expect("parses");
        let b = Hotkey::parse("Ctrl+Alt+T").expect("parses");
    ...
    fn accepts_modifier_aliases() {
        assert_eq!(
            Hotkey::parse("Control+Option+T").expect("parses"),
            Hotkey::parse("Ctrl+Alt+T").expect("parses")
        );
    }

    #[test]
    fn rejects_a_bare_key() {
        assert_eq!(Hotkey::parse("T"), Err(HotkeyError::NoModifier));
    }

    #[test]
    fn rejects_modifiers_alone() {
    ...
    fn rejects_two_non_modifier_keys() {
        assert!(matches!(
            Hotkey::parse("Ctrl+T+Y"),
            Err(HotkeyError::Duplicate(_))
        ));
    }

    #[test]
    fn rejects_a_key_that_does_not_exist() {
        assert_eq!(
    ...
    #[test]
    fn accepts_function_keys_in_range() {
    ...
    #[test]
    fn round_trips_through_its_accelerator() {
    ...
    #[test]
    fn accelerator_uses_tokens_the_plugin_understands() {
        let hotkey = Hotkey::parse("ctrl+shift+g").expect("parses");
        assert_eq!(hotkey.to_accelerator(), "Ctrl+Shift+G");
    }

    #[cfg(not(target_os = "macos"))]
    #[test]
    fn displays_readably_on_desktop_platforms() {
        let hotkey = Hotkey::parse("Ctrl+Alt+T").expect("parses");
        assert_eq!(hotkey.to_string(), "Ctrl + Alt + T");
    }

    #[test]
    fn portal_triggers_use_the_xdg_spelling() {
        assert_eq!(
            Hotkey::parse("Ctrl+Alt+G").unwrap().to_portal_trigger(),
            "CTRL+ALT+g"
        );
    ...
    #[test]
    fn the_archived_apps_suggested_hotkeys_still_parse() {
        for accelerator in ["Ctrl+Alt+T", "Ctrl+Shift+G", "Alt+Shift+F", "Ctrl+Alt+F"] {
            assert!(
                Hotkey::parse(accelerator).is_ok(),
                "{accelerator} must remain valid"
            );
        }
    }
}
