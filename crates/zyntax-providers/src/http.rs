...
pub fn client() -> Result<reqwest::Client, ProviderError> {
    reqwest::Client::builder()
        ...
        .user_agent(concat!("ZyntaxAI/", env!("CARGO_PKG_VERSION")))
        .build()
...
mod tests {
    ...
    fn auth_statuses_map_to_auth() {
        for status in [401, 403] {
            ...
            assert_eq!(error.code(), "auth", "HTTP {status}");
        }
    }
    ...
    fn too_many_requests_maps_to_rate_limited() {
        let error = map_status(ProviderId::Gemini, StatusCode::TOO_MANY_REQUESTS, "");
        assert_eq!(error.code(), "rate_limited");
    }

    #[test]
    fn bad_request_carries_the_providers_explanation() {
        let body = r#"{"error":{"message":"temperature must be <= 2"}}"#;
        let error = map_status(ProviderId::Gemini, StatusCode::BAD_REQUEST, body);
        assert!(error.to_string().contains("temperature must be <= 2"));
    }

    #[test]
    fn not_found_names_the_model_from_the_body() {
        let body = r#"{"error":{"message":"model 'gemini-9-ultra' was not found"}}"#;
        let error = map_status(ProviderId::Gemini, StatusCode::NOT_FOUND, body);
        assert!(error.to_string().contains("gemini-9-ultra"));
    }

    #[test]
    fn not_found_names_an_unquoted_gemini_model() {
        let body = r#"{"error":{"message":"models/gemini-9-does-not-exist is not found for API version v1beta, or is not supported for generateContent."}}"#;
        let error = map_status(ProviderId::Gemini, StatusCode::NOT_FOUND, body);
        assert!(
    ...
    #[test]
    fn not_found_without_a_parseable_body_reads_correctly() {
        let error = map_status(
            ProviderId::Gemini,
    ...
    fn long_error_bodies_are_truncated() {
        ...
        let error = map_status(ProviderId::Gemini, StatusCode::BAD_REQUEST, &body);
        assert!(error.to_string().chars().count() < 500);
    }
